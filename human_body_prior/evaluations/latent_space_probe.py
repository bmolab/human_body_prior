...
def pose_magnitude_feature(pose_body):
    ...
    # VPoser body pose is 21 joints * 3 axis-angle values = 63.
...
def pose_delta_feature(pose_body, previous_pose_body=None):
    ...
    if previous_pose_body is None:
        # The first frame has no previous frame inside the dataset stream.
        delta = torch.zeros(
            pose_body.shape[0], device=pose_body.device, dtype=pose_body.dtype
        )
...
def feature_from_batch(batch, pose_body, feature_name, previous_pose_body):
    """Compute a built-in feature or read a precomputed feature from the dataset."""
    ...
    raise ValueError(
        "Feature '{}' was not found in the dataset batch.".format(feature_name)
    )


def _load_vposer_v1(expr_dir, device):
    """Load the packaged original VPoser v1 model from support_data/dowloads."""
...
def _load_branch_model(expr_dir, device, model_variant):
    """Load a newer Lightning checkpoint for original-layout or 6DOF VPoser."""
    ...
    # On this branch, VPoser is the 6DOF-input model and VPoserOld preserves
    # the original axis-angle encoder for older/non-6DOF checkpoints.
...
def encode_dataset(
    ...
    """Encode every frame from a prepared VPoser split and collect plot features."""
    if model_variant == "v1":
    ...
    # VPoserDS loads one .pt file per field. Built-in features only need
    # pose_body; custom/precomputed features need their own <feature>.pt file.
    data_fields = (
        ["pose_body"]
        if feature_name in ("pose_magnitude", "pose_delta")
        else ["pose_body", feature_name]
    )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            # The current 6DOF branch returns {"latents": Normal(...)}.
            q_z = encoded["latents"] if isinstance(encoded, dict) else encoded
            latents.append(q_z.mean.detach().cpu())
            features.append(
                feature_from_batch(batch, pose_body, feature_name, previous_pose_body)
                .detach()
                .cpu()
            )
...
def project_latents(latents, method):
    """Reduce latent dimensionality to 2D for scatter plotting."""
    if method == "pca":
...
def save_plot(points_2d, feature, out_png, title):
    """Save a 2D latent scatter plot colored by the selected feature."""
    plt.figure(figsize=(8, 6), dpi=150)
    sc = plt.scatter(
        points_2d[:, 0], points_2d[:, 1], c=feature, s=6, cmap="viridis", alpha=0.8
    )
...
def main():
    ...
    parser.add_argument(
        "--dataset-dir",
        required=True,
        help="Prepared VPoser dataset root containing train/vald/test",
    )
    ...
    save_plot(
        ...
        feature,
        osp.join(
            args.out_dir, "latent_probe_{}_{}.png".format(args.split, args.method)
        ),
